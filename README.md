...
6. Audit the mounted result for Pi firmware, VC4/V3D configuration, armed first-boot services, erased factory identity, Quattro session files, required packages, and AArch64 executables.
7. Compress the image and emit exact hashes, a provenance manifest, the audit report, and schema-compatible Raspberry Pi Imager metadata.
8. Boot-test the artifact, then verify graphics and input on a real Pi 4.
