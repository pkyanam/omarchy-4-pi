...
root_fstype="${OMARCHY_ROOT_FSTYPE:-$(findmnt -no FSTYPE / 2>/dev/null || true)}"
if [[ -n $root_fstype && $root_fstype != "btrfs" ]]; then
  echo "Skipping Snapper configuration: the root filesystem is not Btrfs"
else
