...
(
  ...
  grep -F 'ParallelDownloads = 1' "$test_tmp/snapshot-calls" >/dev/null || fail "image installs limit package download pressure"
)
grep -Fx 'Server = http://mirror.archlinuxarm.org/$arch/$repo' "$ROOT/default/pacman/mirrorlist-rpi4-image" >/dev/null || fail "official ARM redirector supplies signed repository databases"
grep -Fx 'Server = https://fl.us.mirror.archlinuxarm.org/$arch/$repo' "$ROOT/default/pacman/mirrorlist-rpi4-image" >/dev/null || fail "official ARM fallback mirror is configured"
grep -Fx 'Server = https://ca.us.mirror.archlinuxarm.org/$arch/$repo' "$ROOT/default/pacman/mirrorlist-rpi4-image" >/dev/null || fail "second official ARM fallback mirror is configured"
! grep -F 'pkgmirror.sametimetomorrow.net' "$ROOT/default/pacman/mirrorlist-rpi4-image" >/dev/null || fail "image no longer depends on the unavailable community snapshot"
grep -Fx 'CacheServer = https://fl.us.mirror.archlinuxarm.org/$arch/$repo' "$ROOT/default/pacman/mirrorlist-rpi4-image" >/dev/null || fail "official mirrors may supply exact cached package files"
grep -F 'ParallelDownloads = 1' "$ROOT/image/build-rpi4-image.sh" >/dev/null || fail "bootstrap limits package download pressure"
grep -Fx 'SigLevel = Required DatabaseOptional' "$ROOT/default/pacman/pacman-rpi4.conf" >/dev/null || fail "rolling ARM packages still require trusted signatures"
grep -F '"package_repository":' "$ROOT/image/build-rpi4-image.sh" >/dev/null || fail "manifest records its package repository"
pass "official rolling package sources are image-only and retain signature enforcement"
